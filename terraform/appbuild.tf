...
resource "null_resource" "recipe-api_springboot" {
  provisioner "local-exec" {
    command = <<EOT

	cd ../ && mvn clean package -Dmaven.test.skip=true
    	docker build -t recipe-api . 
    	docker tag recipe-api ${data.aws_ecr_repository.image_repo.repository_url}
...
depends_on = [aws_ecr_repository.recipe-api]
}